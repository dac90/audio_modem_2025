...
def generate_psuedo_random_signal(block_size, num_blocks):
    ...
    for i in range(num_blocks):
        ...
        for k in range(1, int(len(QPSK_symbols_trans) // 2 + 1)):
            assert QPSK_symbols_trans[k] == np.conjugate(QPSK_symbols_trans[len(QPSK_symbols_trans) - k]), (
                f"QPSK symbol {k} is not conjugate to QPSK symbol {len(QPSK_symbols_trans) - k - 1}"
            )
